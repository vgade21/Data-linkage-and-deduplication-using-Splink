...
-- Updating table DO_NOT_MIGRATE_person_dedup with the original given_names and surname.
-- Update DO_NOT_MIGRATE_person_dedup with data from person using id as the link
-- 75,651 records updated
UPDATE d
SET
    d.given_name_1 =
        CASE
...
FROM
    fvms_staging.dbo.DO_NOT_MIGRATE_person_dedup d
JOIN
    fvms_clean.dbo.person c
    ON d.id = c.id -- (change join as appropriate)

PRINT 'Updating table DO_NOT_MIGRATE_person_dedup: ' + CAST(@@ROWCOUNT AS VARCHAR) + ' records updated.';
PRINT '';
...
FROM fvms_staging.dbo.DO_NOT_MIGRATE_person_dedup f
INNER JOIN NewClusterId nci
    ON f.cluster_id = nci.cluster_id;

PRINT 'Updating table DO_NOT_MIGRATE_person_dedup with an updated_cluster_id, as per User Story 37973: ' + CAST(@@ROWCOUNT AS VARCHAR) + ' records updated.';
PRINT '';

GO

-- 75,651 records - person
