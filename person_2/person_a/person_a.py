...
class Person_a:
    ...
    def upload(self, conn: DatabaseConnector):
        ...
        sql_ops.write_to_db_bcp(self.person_lookup, "DO_NOT_MIGRATE_person_dedup", types=col_type)
        self.clean_via_sql()

    def clean_via_sql(self):
        # ADD new sql script here
        sql_file_name = ["fv_dedup_per.sql"]
    ...
    def dedup_person(self, df_new_duplicates_exact: pd.DataFrame, df_training_data: pd.DataFrame) -> pd.DataFrame:
        ...
        The resulting clusters_pandas_df_5 contains:
        ...
        settings = "./splink_models/fvms_person_model_1e.json"
        ...
        # 19,919 records (pairwise comparisons above the threshold)
        ...
        # 73,762 records - training data (first duplicated (exact) row + rest)
        # 1,889 records - exact match data (remaining duplicated (exact) rows)
        logger.info("")
        logger.info(
        ...
        # Unique count of cluster_id: 66,048
        # Percentage reduction: 12.69%
