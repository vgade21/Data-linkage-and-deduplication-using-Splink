...
# Input: Original person table along with suburb and contact_number (actual number).
...
# Output: a mapping table (DO_NOT_MIGRATE_person_dedup) between the person table ids and master_id (updated_cluster_id).
...
WITH PersonLink AS (
    SELECT
        l.*,
        ROW_NUMBER() OVER (PARTITION BY l.id_dbo_person ORDER BY l.incident_id_dbo_involved_person DESC) AS rn
    FROM
...
CTE_person_data AS (
	SELECT
		...
		p.ethnicity_other,
		-- Split given_names
		LEFT(p.given_names, CHARINDEX(' ', p.given_names + ' ') - 1) AS given_name_1,
		CASE
			WHEN CHARINDEX(' ', p.given_names) > 0
			THEN LEFT(
					STUFF(p.given_names, 1, CHARINDEX(' ', p.given_names), ''),
					CHARINDEX(' ', STUFF(p.given_names, 1, CHARINDEX(' ', p.given_names), '') + ' ') - 1
				)
			ELSE NULL
		END AS given_name_2,
		CASE
			WHEN CHARINDEX(' ', p.given_names) > 0
				AND CHARINDEX(' ', STUFF(p.given_names, 1, CHARINDEX(' ', p.given_names), '')) > 0
			THEN LTRIM(
					STUFF(
						STUFF(p.given_names, 1, CHARINDEX(' ', p.given_names), ''),
						1, CHARINDEX(' ', STUFF(p.given_names, 1, CHARINDEX(' ', p.given_names), '')), ''
					)
				)
			ELSE NULL
		END AS given_name_3,
		...
		a.suburb
	FROM
		fvms_clean.dbo.person p
		LEFT JOIN PersonLink l
			ON p.id = l.id_dbo_person AND l.rn = 1
...
# 73,721 records (75,651 - 73,721 = 1,930 duplicates removed) and 17 columns
...
"""
michael b - 12544 comparisons being generated
j smith - 18496 comparisons being generated
1990-01-01 - 7921 comparisons being generated
"""
...
else_comparison_level._m_probability = 0.04
...
exact_comparison_level._m_probability = 0.1

field_comparison = linker._settings_obj._get_comparison_by_output_column_name("sex")
exact_comparison_level = field_comparison._get_comparison_level_by_comparison_vector_value(0)
exact_comparison_level._m_probability = 0.008
...
model_name = "fvms_person_model_1e"
...
settings = "./temp_db/fvms_person_model_1.json"
linker = Linker(df_training_data_3, settings, db_api=DuckDBAPI())
...
# 19,133 records (pairwise comparisons above the threshold)
...
df_clusters = linker.clustering.cluster_pairwise_predictions_at_threshold(  # noqa
    df_predictions, threshold_match_weight=18.5
)
# Converting to a Pandas dataframe
# 73,762 records (same number of records as df_training_data_2)
...
73,762 - person records original
66,062 - person records after deduplication
7,700 - person records merged duplicates (10% reduction)

75,651 - person records actual
1,889 - person records exact duplicates
9,589 - person records duplicates total (12% reduction)
"""
...
# 75,651 records and 19 columns (same as total record count for cleaned person table)
clusters_pandas_df_2 = process_clusters(df_new_duplicates_exact_2, clusters_pandas_df)

# Unique count of id: 75,651
# Unique count of cluster_id: 66,065
# Percentage reduction: 12.67%
...
# 75,651 records and 37 columns
# 59 records as "Association match"
# 1,886 records as "Association match (Exact)"
...
# 75,651 records - DO_NOT_MIGRATE_person_dedup
# 66,065 records - person
# 12.67% reduction
