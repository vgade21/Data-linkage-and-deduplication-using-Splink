...
# Script Purpose: build and train a Splink model (based on Fellegi-Sunter's model of record linkage) for probabilistic record linkage that allows us to link witness records and de-duplicate them.
#                 https://moj-analytical-services.github.io/splink/index.html
# Feature DQI: 24183, 35420, 37973, 24127
# Input: Original witness table (selected columns) along with suburb and contact_number (actual number))
# Output: a mapping table (DO_NOT_MIGRATE_witness_dedup) between the witness table ids and master_id (updated_cluster_id).
...
WITH PersonLink AS (
    SELECT
        l.*,
        ROW_NUMBER() OVER (PARTITION BY l.id_dbo_involved_person ORDER BY l.incident_id_dbo_involved_person DESC) AS rn
    FROM fvms_clean.dbo.DO_NOT_MIGRATE_Names_LinkTable l
    INNER JOIN fvms.dbo.witness w
        ON l.id_dbo_involved_person = w.id
),
CTE_person_data AS (
	SELECT
		p.id,
	...
	FROM
		...
		LEFT JOIN PersonLink l
			ON p.id = l.id_dbo_involved_person AND l.rn = 1
...
# 6,333 records (8,429 - 6,333 = 2,096 exact duplicates removed) and 13 columns
...
"""
peter c - 144 comparisons being generated
s smith - 196 comparisons being generated
2006-05-22 - 36 comparisons being generated
"""
...
1e9 - memory error
"""
linker.training.estimate_u_using_random_sampling(max_pairs=7e8)
...
training_blocking_rules = [
    block_on("substr(given_name_1, 1, 1)"),
...
model_name = "fvms_witness_model_1a"
...
settings = "./temp_db/fvms_witness_model_1a.json"
linker = Linker(df_training_data_3, settings, db_api=DuckDBAPI())
...
# 289 records (pairwise comparisons above the threshold)
...
given_name_2_l, given_name_2_r, date_of_birth_l, date_of_birth_r, contact_number_l, contact_number_r,
suburb_l, suburb_r
FROM data
...
df_clusters = linker.clustering.cluster_pairwise_predictions_at_threshold(  # noqa
    df_predictions, threshold_match_weight=5.8
)
# Converting to a Pandas dataframe
# 6,334 records (same number of records as df_training_data_2)
...
6,334 - witness records original
6,079 - witness records after deduplication
255 - witness records merged duplicates (0.04% reduction)

8,429 - witness records actual
2,095 - witness records exact duplicates
2,350 - witness records duplicates total (27.8% reduction)
"""
...
# 8,429 records and 14 columns (same as total record count for cleaned witness table)
clusters_pandas_df_2 = process_clusters(df_new_duplicates_exact_2, clusters_pandas_df)

# Unique count of id: 8,429
# Unique count of cluster_id: 6,079
# Percentage reduction: 27.88%
...
# 8,429 records and 28 columns
# 3 records as "Association match"
# 2,095 records as "Association match (Exact)"
...
# 8,429 records - DO_NOT_MIGRATE_witness_dedup
# 6,079 records - witness
# 27.88% reduction
