...
# Script Purpose: Use a trained linking model to link witness records and de-duplicate them.
#                 https://moj-analytical-services.github.io/splink/index.html
# Feature DQI: 24183, 35420, 37973
# Input: Original data from witness and involved_person tables along with suburb and contact_number (actual number)
#        The witness data is split into exact matches (df_new_duplicates_exact) and the rest (df_training_data), we apply the linking model to the rest of the data.
...
class Person_d:
    ...
    def process(self, df):
        self.df = df
        self.person_lookup = self.dedup_witness(self.df_new_duplicates_exact, self.df_training_data)
    ...
    def upload(self, conn: DatabaseConnector):
        ...
        sql_ops.write_to_db_bcp(self.person_lookup, "DO_NOT_MIGRATE_witness_dedup", types=col_type)
        self.clean_via_sql()

    def clean_via_sql(self):
        # ADD new sql script here
        sql_file_name = ["fv_dedup_witness.sql"]
    ...
    def dedup_witness(self, df_new_duplicates_exact: pd.DataFrame, df_training_data: pd.DataFrame) -> pd.DataFrame:
        """
        Merging the clustered witness records with the exact match wintess records.
        ...
        settings = "./splink_models/fvms_witness_model_1a.json"
        ...
        # Converting to a Pandas dataframe
        # 6,333 records (same number of records as df_training_data_2)
        clusters_pandas_df = df_clusters.as_pandas_dataframe()

        # 6,333 records - training data (first duplicated (exact) row + rest)
        # 2,096 records - exact match data (remaining duplicated (exact) rows)
        logger.info("")
        logger.info(
