...
-- Updating table DO_NOT_MIGRATE_witness_dedup with the original given_names and surname.
-- Update DO_NOT_MIGRATE_witness_dedup with data from involved_person using id as the link
-- 8,429 records updated
UPDATE d
SET
    d.given_name_1 =
        CASE
	...
	-- 	))
FROM
    fvms_staging.dbo.DO_NOT_MIGRATE_witness_dedup d
JOIN
    fvms_clean.dbo.involved_person c
    ON d.id = c.id -- (change join as appropriate)

PRINT 'Updating table DO_NOT_MIGRATE_witness_dedup: ' + CAST(@@ROWCOUNT AS VARCHAR) + ' records updated.';
PRINT '';
...
FROM fvms_staging.dbo.DO_NOT_MIGRATE_witness_dedup f
INNER JOIN NewClusterId nci
    ON f.cluster_id = nci.cluster_id;

PRINT 'Updating table DO_NOT_MIGRATE_witness_dedup with an updated_cluster_id, as per User Story 37973: ' + CAST(@@ROWCOUNT AS VARCHAR) + ' records updated.';
PRINT '';

GO

-- 8,429 records - witness
